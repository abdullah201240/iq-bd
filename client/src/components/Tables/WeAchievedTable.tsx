...
const WeAchievedTable = () => {
    ...
    const [currentteam, setCurrentteam] = useState<WeAchieved | null>(null);
    const [formValues, setFormValues] = useState<WeAchieved>({
        id: 0,
        title: '',
        subTitle: '',
        image: null,
        ...
        const checkSession = async () => {
            ...
            try {
                ...
                const teamResponse = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL}/admin/viewWeAchieved`,
                    {
                        method: 'GET',
                        headers: {
                            Authorization: `Bearer ${storedUserInfo}`,
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/admin/deleteWeAchieved/${id}`,
                {
                    method: 'DELETE',
                    headers: {
                        Authorization: `Bearer ${storedUserInfo}`,
    ...
    // Handle edit
    const handleEdit = (team: WeAchieved) => {
        setCurrentteam(team);
        setFormValues(team);
        setIsModalOpen(true);
    };
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/admin/weAchieved/${formValues.id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${storedUserInfo}`,
            ...
            <div className="flex flex-col text-white">
                ...
                {services.length > 0 ? (
                    services.map((service) => (
                        <div
                            className={`grid grid-cols-7 sm:grid-cols-5 ${services.indexOf(service) === services.length - 1
                                    ? ''
                        ...
                        >
                            ...
                            <div className="flex items-center justify-center p-2.5 xl:p-5">
                                <p className="text-black">{service.date}</p>
                            </div>
...
export default WeAchievedTable;
